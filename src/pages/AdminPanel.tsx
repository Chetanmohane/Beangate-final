...
const ReferralTab = () => {
  ...
  const [editApplicablePlan, setEditApplicablePlan] = useState<"all" | "one-time" | "installment">("all");

  const startEditCode = (codeObj: RefCode) => {
    setEditingCode(codeObj);
    setEditDiscountPercent(codeObj.discountPercent || parseInt(codeObj.discount) || 10);
    setEditApplicablePlan(codeObj.applicablePlan || "all");
  };

  const handleSaveEditedCode = async () => {
    if (!editingCode) return;
    const discPct = Number(editDiscountPercent) || 10;
    const updatedObj: RefCode = {
      ...editingCode,
      discount: `${discPct}%`,
      discountPercent: discPct,
      applicablePlan: editApplicablePlan
    };

    const newCodes = codes.map(c => c.code === editingCode.code ? updatedObj : c);
    setCodes(newCodes);
    ...
    const id = (editingCode as any)._id;
    if (id) {
      try {
        ...
        console.error("Failed to update refcode on API:", err);
      }
    }
    setEditingCode(null);
  };
    ...
    <div>
          ...
          return (
            <Card key={i} className="p-5 space-y-3.5 border border-slate-100 dark:border-white/5">
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => startEditCode(c)} className="p-2 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 rounded-lg transition border-none cursor-pointer flex items-center justify-center">
              ...
              {filteredCodes.map((c, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-slate-100 dark:border-white/5 hover:bg-slate-50/50 dark:hover:bg-white/5 transition">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => startEditCode(c)} className="text-slate-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition bg-transparent border-none cursor-pointer p-1">
      ...
      {editingCode && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#0e1726] border border-slate-200 dark:border-white/10 rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-5">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-4">
              <div>
                <h3 className="text-lg font-black text-slate-900 dark:text-white">Edit Referral Code</h3>
                <p className="text-xs text-indigo-500 font-mono font-bold">{editingCode.code}</p>
              </div>
              <button onClick={() => setEditingCode(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-white bg-transparent border-none cursor-pointer p-1">
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-4 gap-2 mb-2">
                  {[7, 10, 15, 20].map(pct => (
                    <button
                      key={pct}
                      type="button"
                      onClick={() => setEditDiscountPercent(pct)}
                      className={`py-2 rounded-xl text-xs font-bold border transition cursor-pointer ${editDiscountPercent === pct ? "bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-500/20" : "bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-white/10 hover:bg-indigo-50"}`}
                    >
                      {pct}% OFF
                    </button>
                  ))}
                ...
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500"
                  placeholder="Custom Discount % (e.g. 15)"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">Applicable Plan</label>
                <select
                  value={editApplicablePlan}
                  onChange={e => setEditApplicablePlan(e.target.value as any)}
                  className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500"
                >
                  <option value="all">All Plans (One-Time & Installments)</option>
                  <option value="one-time">One-Time Plan Only</option>
                  <option value="installment">Installment Plan Only</option>
                </select>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <Button onClick={() => setEditingCode(null)} variant="secondary" className="flex-1">
                Cancel
              </Button>
              <Button onClick={handleSaveEditedCode} variant="primary" className="flex-1">
                Save Changes
              </Button>
            </div>
          </div>
